...
class ExtractiveQA:
    def __init__(self, document_store):
        ...
        self.reader = TransformersReader(
            model_name_or_path=self.config["extractive_model_name"]
        )

    def answer(self, question, document_hash):
        # Filter document store based on hash
        filtered_document_store = filter_document_store(
            document_store=self.document_store, document_hash=document_hash
        )

        # Execution
        retrieved_documents = self.retriever.retrieve(
            query=question, document_store=filtered_document_store
        )
